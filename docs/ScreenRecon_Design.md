...
| 2026-08-17 | Restore the wizard's numbered picker (SR-38). The SR-36 free-form text entry lost the "see the alternatives at a glance and Enter to keep" affordance — at the terminal the current prompt appeared inside `[brackets]` on one long line with no menu, and `examples/prompts.json` was never loaded, so users couldn't discover the alternatives without opening a browser. Bring `PROMPT_CHOICES` back with six flat entries (`describe`, `answer`, `translate_english`, `find_errors`, `table_to_csv`, `extract_text`), reintroduce `_ask_prompt` as a one-line wrapper around `_ask_choice` (no code template / language substitution this time), and wire it back into both `run_set_prompt` and wizard step 4. Delete `examples/prompts.json` — the shortlist is discoverable in-terminal again and the file duplicated the shipped defaults. `_ask_choice`'s "keep current" line changes from `N) (keep current — <preview>)` to `N) Current <label> (<matching preset's note or preview>)`, so when the current value already matches a shipped preset the numbered entry and the current line share the same descriptive note — the two options visibly resolve to the same result. Applies to every `_ask_choice` caller (prompt picker, model picker, provider picker). Inline `--prompt "..."` from SR-36 stays. |
| 2026-08-17 | Follow-up wizard polish (SR-40). Drop the leading spaces from every `_ask` / `_ask_float` / `_ask_choice` label so the input line sits at column 0 while numbered option rows stay indented four spaces — the input and its option context are now visually distinct. Capitalize the first letter of `model ID`, `dwell seconds`, `save directory`, `model`, and `prompt`; labels that already began with a capital (`API key`, `Telegram bot token`, `AI model`, `AI provider`, `Update this region?`) are unchanged. Applies to the main wizard and every single-field setter. |
...
monitor, via `display.describe_region_monitor`. Multi-monitor users see at a
glance which display their region belongs to.
...
or picker the wizard uses for that one field, saves, and exits. They cover
every field the wizard writes, so a user who wants to change one thing
never has to re-run the full wizard (and re-enter unrelated credentials to
do it). `--telegram` is the only setter that touches two fields together:
the bot token and chat ID are a matched pair, and splitting them would let
a valid token be reused with the wrong chat ID and silently misdeliver
captures. All setters refuse to run if no config exists yet (the user needs
to have gone through `--configure` at least once). Everything except the
field being set is preserved byte-for-byte, so a partial config that omits
fields the user has not set yet stays partial — these flags never silently
pad the file with defaults. Mutually exclusive: one setter at a time.
...
1. **`release_pr.yml`** (trigger: `workflow_dispatch`, `bump` input =
   ...
   rest.
2. **`release_auto.yml`** (trigger: `push` to `main`) — inspects the head
