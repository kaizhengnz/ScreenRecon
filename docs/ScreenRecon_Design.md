...
`--screen`, `--key`, and `--model` are targeted single-field setters — each
opens the same prompt or picker the wizard uses for that one field, saves,
and exits. All three refuse to run if no config exists yet (the user needs
to have gone through `--configure` at least once so credentials are present).
Everything except the field being set is preserved byte-for-byte, so a
partial config that omits fields the user has not set yet stays partial —
these flags never silently pad the file with defaults. Mutually exclusive:
one field at a time.
