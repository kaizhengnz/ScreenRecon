...
def test_new_stem_treats_png_and_txt_as_a_pair(tmp_path):
...
def test_save_png_and_txt_round_trip(tmp_path):
    directory = storage.resolve_dir(tmp_path / "archive")
    png = storage.save_png(directory, "shot", b"\x89PNG-data")
    txt = storage.save_txt(directory, "shot", "recognised text")

    assert png is not None and png.read_bytes() == b"\x89PNG-data"
...
def test_save_leaves_no_temporary_files_behind(tmp_path):
    directory = storage.resolve_dir(tmp_path / "archive")
    storage.save_png(directory, "shot", b"data")
    assert sorted(p.name for p in directory.iterdir()) == ["shot.png"]
...
def test_save_failure_warns_and_returns_none(tmp_path, capsys, monkeypatch):
    ...
    assert storage.save_png(directory, "shot", b"data") is None
...
def test_archive_is_owner_only(tmp_path):
    ...
    directory = storage.resolve_dir(tmp_path / "archive")
    png = storage.save_png(directory, "shot", b"data")
